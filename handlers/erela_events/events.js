        ...
        .on("trackStart", async (player, track) => {
          try {
            ...
            var embed = new MessageEmbed().setColor(ee.color)
              embed.setTitle(`**<a:playingsong:860516395133435904> | ${track.title}**`)
              ...
              embed.addField(`Queue Status`, `**🎤Join Channel:** \`#${client.channels.cache.get(player.textChannel).name}\``)
              embed.addField(`🔊 Volume`, `${player.volume}%`, true)
              embed.addField(`${player.queueRepeat ? `♾ Queue Loop: ` : `♾ Song Loop: `}`, `${player.queueRepeat ? `👍 Enabled` : player.trackRepeat ? `$👍 Enabled` : `❌ Disabled`}`, true)
              embed.addField(`♾ Autoplay`, `${player.get(`autoplay`) ? `👍 Enabled` : `❌ Disabled`}`, true)
              embed.addField(`💰 24/7 AFK Setup`, `PLAYER: ${player.get(`afk-${player.get(`playerauthor`)}`) ? `👍 Enabled` : `❌ Disabled`}\nGUILD: ${player.get(`afk-${player.guild}`) ? `$👍 Enabled` : `❌ Disabled`}`, true)
